...
def cli(before, packages):
    ...
    print(''.join('{}=={}\n'.format(p['package'], str(p['release_version'])) for p in required_packages))
    if len(not_found) > 0:
        print('\nCould not find the following packages: {}'.format(' '.join(p for p in not_found)))

if __name__ == '__main__':
    cli()
    # python versions.py tensorflow numpy pillow scikit-image opencv-python --before 2018-06-21 > requirements.txt