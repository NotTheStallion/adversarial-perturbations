...
def main(args):
    ...
    names = (
        open(os.path.join(args.centroids, "centroids_names.txt"))
        .read()
        .split("\n")[:-1]
    )

    IDcolor = [255.0, 255.0, 255.0]
    IDcolor2 = [255.0, 0.0, 0.0]

    # video_capture = cv2.VideoCapture(0)
    # video_capture.set(3, 1280)
    # video_capture.set(4, 1024)
    # read image instead of video
    image_capture = cv2.imread("data/demo_universal/test_img.jpg")
    ...
    while True:
        ...
        if batch.shape[0] != 0:
            embs = sess.run(
                embedding,
                feed_dict={image_input: batch, keep_prob: 1.0, is_train: False},
            )
            for i in range(bounding_boxes.shape[0]):
                ...
                cv2.putText(
                    frame,
                    "Sim. to top-1 class: " + str(round(val, 4)),
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "centroids", type=str, help="Dir with centoids of classes for classifier."
    )
