    ...
    with tf.device(device):
        ...
        if os.path.isfile(file_perturbation) == 0:
            ...
            v = universal_perturbation(
                X, f, grad_fs, delta=0.2, num_classes=num_classes
            )
        ...
        clipped_v = np.clip(
            undo_image_avg(image_original[0, :, :, :] + v[0, :, :, :]), 0, 255
        ) - np.clip(undo_image_avg(image_original[0, :, :, :]), 0, 255)
        ...
        plt.subplot(1, 3, 1)
        plt.imshow(
            undo_image_avg(image_original[0, :, :, :]).astype(dtype="uint8"),
            interpolation=None,
        )
        ...
        plt.subplot(1, 3, 3)
        plt.imshow(
            undo_image_avg(v[0, :, :, :]).astype(dtype="uint8"),
            interpolation=None,
        )
        plt.title("Perturbation v")
